...
from app.models import (
    BucketAnalysis, BucketRequest, CompareRequest,
...
app = FastAPI(title="Flight Price Monitor", version="0.2.0", lifespan=lifespan)
...
async def duffel_error_handler(request, exc: DuffelError):
    return JSONResponse(status_code=exc.status_code or 502,
        content=ErrorResponse(error="Duffel API error", detail=str(exc), code=exc.code or "duffel_error").model_dump())

@app.exception_handler(ValueError)
async def value_error_handler(request, exc: ValueError):
    return JSONResponse(status_code=422, content=ErrorResponse(error="Validation error", detail=str(exc)).model_dump())
...
async def manual_run(background_tasks: BackgroundTasks, duffel: DuffelClient = Depends(require_duffel)):
    ...
    return {"status": "started", "message": "Daily check running in background"}
...
async def price_history(route_id: str, limit: int = Query(default=30, ge=1, le=90)):
    ...
    return {"route_id": route_id, "history": get_price_history(route_id, limit)}

@app.post("/search", response_model=SearchResponse, tags=["search"])
async def search(request: SearchRequest, max_results: int = Query(default=10, ge=1, le=50), duffel: DuffelClient = Depends(require_duffel)) -> SearchResponse:
    from datetime import datetime
    offers = await duffel.create_offer_request(request)
    return SearchResponse(offers=offers[:max_results], total_results=len(offers), searched_at=datetime.utcnow().isoformat(), cabin=request.cabin, trip_type=request.trip_type)

@app.post("/compare", response_model=CompareResult, tags=["compare"])
async def compare(request: CompareRequest, engine: CompareEngine = Depends(get_engine), _duffel: DuffelClient = Depends(require_duffel)) -> CompareResult:
    return await engine.compare(request)

@app.post("/compare/buckets", response_model=BucketAnalysis, tags=["compare"])
async def bucket_analysis(request, engine: CompareEngine = Depends(get_engine), _duffel: DuffelClient = Depends(require_duffel)) -> BucketAnalysis:
    return await engine.bucket_analysis(request)

@app.get("/hubs/suggest", tags=["hubs"])
async def suggest_hub_airports(origin: str = Query(..., min_length=3, max_length=3), destination: str = Query(..., min_length=3, max_length=3), dest_flag: Optional[str] = Query(default=None), max_hubs: int = Query(default=8, ge=1, le=12)) -> dict:
    hubs = suggest_hubs(origin=origin.upper(), destination=destination.upper(), dest_flag=dest_flag, max_hubs=max_hubs)
